...
function clearNames(url) {
  return url.toString().replace(cityscope_server+"/api/table/", "");
}
////////////////////////////////////////////////////////////////////////////////////
...
async function getTables() {
  let counter = 0;
  let tableArray = [];
  let cityIOurl = cityscope_server+"/api/tables/list";
...
function makeMap(tablesArray, counter) {
  ...
  async function modalSetup(m) {
    ...
    //put prj name in div
    let delLink = cityscope_server+"/api/table/clear/" + tableMeta.name;
...
// getCityIO(cityscope_server+"/api/table/hidden_table", myHeaders);
