...
#Code for finding cityscope blocks
...
def find_matrix_points(gdf,h,w):
    ...
    for i in points:
        temp_points = []
        for j in i:
            temp_points.append(transfrom_latlng_to_m(j[0],j[1],gdf.total_bounds,h,w))
        block_points.append(temp_points)
    return block_points

def reduce_by_05(bounds):
    a1 = [bounds[0][0][0][0]+0.05,bounds[0][0][0][1]+0.05]
    a2 = [bounds[0][0][1][0]+0.05,bounds[0][0][1][1]-0.05]
    a3 = [bounds[0][0][2][0]-0.05,bounds[0][0][2][1]-0.05]
    a4 = [bounds[0][0][3][0]-0.05,bounds[0][0][3][1]+0.05]
    return([[[a1,a2,a3,a4]]])

def find_block_json(gdf,grid,h,w):
    ...
    for y in range(0,ylen):
        for x in range(0,xlen):
            ...
            print(y,x)
            #bounds = [[[[matrix[0][x][0],matrix[1][y][1]],[matrix[0][x+1][0],matrix[1][y][1]],[matrix[0][x+1][0],matrix[1][y+1][1]],[matrix[0][x][0],matrix[1][y+1][1]],[matrix[0][x][0],matrix[1][y][1]]]]]
            bounds = reduce_by_05([[[[matrix[1][x][1],matrix[0][y][0]],[matrix[1][x][1],matrix[0][y+1][0]],[matrix[1][x+1][1],matrix[0][y+1][0]],[matrix[1][x+1][1],matrix[0][y][0]],[matrix[1][x][1],matrix[0][y][0]]]]])
            centroid = [(matrix[1][x][1]+matrix[1][x+1][1])/2,(matrix[0][y][0]+matrix[0][y+1][0])/2]
            geom1 = {"centroids": [[centroid[0],centroid[1],0.0]],"h":grid[y][x][1],"cat":dict_type[grid[y][x][0]],"polygons":bounds}
            feature["geom"] = geom1
            features.append(feature)
    json1["features"] = features
    return(json1)
...
def run_all(bounds,h,w,name,email):
    ...
    cityscopejson = get_cityscope_json(gdf,name,email)
    ...
    blocks = find_block_json(gdf,cityscopejson["grid"],h,w)
    return(image,geojson,q3jsjson,cityscopejson,blocks)
